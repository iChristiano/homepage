...
exports.onCreatePage = ({ page, actions }) => {
    ...
    Object.keys(locales).map(lang => {
        // update path for tag page
        const path = page.path === '/tags/' ? '/blog' + page.path : page.path;

        // Use the values defined in "locales" to construct the path
        const localizedPath = locales[lang].default ? path : `${locales[lang].path}${path}`;
...
exports.createPages = ({ actions, graphql }) => {
    ...
    `).then(result => {
        ...
        const contentMarkdown = result.data.allMarkdownRemark.edges;
        ...
        contentMarkdown.forEach(edge => {
            ...
            const isPage = edge.node.frontmatter.templateKey === 'blog-post' ? false : true;
